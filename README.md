...
# DTO vs JSON(직렬화)
## DTO(Data Transfer Object)
- 데이터를 한 번에 전달하기 위해 만들어 진 클래스가 DTO
- 객체로 정리된 DTO일 경우 외부 통신을 하기 위한 직렬화에 대한 로직은 내부에 캡슐화하여 가지고 있을 수 있기 떄문에 코드가 명확해진다. 
...
- 디스크에 저장하거나 통신할 때는 값 형식의 데이터만 가능 !
### 직렬화의 결과
- 직렬화를 하게 되면 각 주소값들이 가지는 데이터를 모두 끌어보아 값 형식 데이터로 변환.
